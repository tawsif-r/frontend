...
export type Column<T> = {
  header: string;
  accessor: keyof T;
  render?: (value: T[keyof T], row: T, updateRow?: (updatedRow: Partial<T>) => void) => React.ReactNode;
};
...
const Table = <T extends { id: number }>({
  ...
  // Sync editRow with editingData only when editing starts
  useEffect(() => {
    if (editingData && !editRow) {
      setEditRow(editingData); // Only set if editRow is null (i.e., editing just started)
    }
  }, [editingData]);
  ...
  const handleFieldUpdate = (updatedFields: Partial<T>) => {
    setEditRow((prev) => {
      if (!prev) return null;
      const newRow = { ...prev, ...updatedFields };
      console.log("Updated editRow:", newRow);
      return newRow;
    });
  };
          ...
          {data.map((row) => (
            <tr key={row.id}>
              {columns.map((column, colIndex) => {
                const isEditing = editingData?.id === row.id;
                const value = isEditing && editRow ? editRow[column.accessor] : row[column.accessor];
                ...
                return (
                  ...
                  >
                    {isEditing && column.render
                      ? column.render(value, row, handleFieldUpdate)
                      : column.render
              ...
              <td className="px-6 py-2 border-b border-gray-700 bg-slate-900">
                {editingData?.id === row.id ? (
                  <>
                    <button className="mr-2 px-2 py-1" onClick={handleSave}>
                      <svg
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth="1.5"
                        stroke="currentColor"
                        className="size-6 hover:scale-150 transition-all duration-300"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                      </svg>
                    </button>
                    <button className="mr-2 px-2 py-1" onClick={handleCancel}>
                      <svg
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
